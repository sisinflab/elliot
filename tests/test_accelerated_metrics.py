...
from elliot.evaluation.metrics import Precision, Recall, HR, MRR, MAP, MAR, F1, nDCG
...
def _legacy_results(recommendations, test_data, cutoff, threshold):
    eval_objects = SimpleNamespace(
        ...
        num_items=100,
        data=SimpleNamespace(get_train_dict=lambda: {}),
    )
    metric_classes = [Precision, Recall, HR, MRR, MAP, MAR, F1, nDCG]

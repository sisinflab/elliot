...
class KnowledgeAwareDataSet(DataSet):
    ...
    def __init__(self,
        ...
        self.num_users = len(self.users)
        self.items = list({k for a in self.train_dataframe_dict.values() for k in a.keys() if k in self.feature_map.keys()})
        self.num_items = len(self.items)
        self.transactions = sum(len(v) for v in self.train_dataframe_dict.values())

        # self.train = self.build_sparse()
    ...
    def build_sparse(self):
        rows_cols = [(u, i) for u, items in self.train_dataframe_dict.items() for i in items.keys()]

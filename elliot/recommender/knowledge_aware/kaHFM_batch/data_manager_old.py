...
class KnowledgeAwareDataSet(DataSet):
    """
    ...
    def __init__(self,
        ...
        self.train_dataframe_dict, self.feature_map = self.load_dataset_dict(path_train_data,
                                                                             "\t",
                                                                             path_map,
                                                                             path_features,
                                                                             path_properties)
        self.users = list(self.train_dataframe_dict.keys())
        self.num_users = len(self.users)
        self.items = list({k for a in self.train_dataframe_dict.values() for k in a.keys()})
        ...
        self.transactions = sum(len(v) for v in self.train_dataframe_dict.values())
        ...
        self.indexed_ratings = {self.public_users[user]: {self.public_items[i]: v for i,v in items.items()}
                                for user, items in self.train_dataframe_dict.items()}
    ...
    def build_sparse(self):
        ...
        rows_cols = [(u, i) for u, items in self.indexed_ratings.items() for i in items.keys()]
    ...
    def step(self, events: int, batch_size: int):
        ...
        indexed_ratings = self.indexed_ratings

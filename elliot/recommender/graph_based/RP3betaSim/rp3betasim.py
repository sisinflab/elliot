...
from similaripy import similarity as sim


class RP3betaSim(RecMixin, BaseRecommenderModel):
    ...
    def train(self):
        ...
        W_sparse = sim.rp3beta(self._data.sp_i_train_ratings.T, k=self._neighborhood, alpha=self._alpha,
                               beta=self._beta, format_output='csr')
        if self._normalize_similarity:

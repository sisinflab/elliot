    ...
    def __init__(self, channels, kernels, strides, name="ConvolutionalComponent", **kwargs):
        ...
        for i in range(self.num_of_nets):
            self.cnn_network.add(
                keras.layers.Conv1D(kernel_size=self.kernels[i], filters=self.channels[i + 1], strides=self.strides[i],
                                    use_bias=True,
                                    activation='relu',
...
class ConvMatrixFactorizationModel(keras.Model):
    def __init__(self,
    ...
    def call(self, inputs, training=False, **kwargs):
        ...
        interaction_map = tf.concat([user_mf_e, item_mf_e], axis=1)  # batch_size x embedding_size*2
        interaction_map = tf.expand_dims(interaction_map, axis=2)
        conv_layers_e = self.conv_layers(interaction_map)
        conv_layers_e = tf.reduce_sum(conv_layers_e, axis=(1))
    ...
    def get_recs(self, inputs, training=False, **kwargs):
        ...
        interaction_map = tf.concat([user_mf_e, item_mf_e], axis=2)  # batch_size x embedding_size*2
        interaction_map = tf.expand_dims(tf.squeeze(interaction_map, axis=1), axis=2)
        conv_layers_e = self.conv_layers(interaction_map)
        conv_layers_e = tf.reduce_sum(conv_layers_e, axis=(1))

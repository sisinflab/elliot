...
__paper__ = 'Outer Product-based Neural Collaborative Filtering [https://www.ijcai.org/Proceedings/2018/0308.pdf]'
...
class ConvNeuMF(RecMixin, BaseRecommenderModel):
    ...
    def __init__(self, data, config, params, *args, **kwargs):
        ...
        self._params_list = [
            ...
            ("_cnn_channels", "cnn_channels", "cnn_channels", "(1, 32, 32)", lambda x: list(make_tuple(x)),
             lambda x: str(x).replace(",", "-")),
            ("_cnn_kernels", "cnn_kernels", "cnn_kernels", "(2,2)", lambda x: list(make_tuple(x)),
             lambda x: str(x).replace(",", "-")),
            ("_cnn_strides", "cnn_strides", "cnn_strides", "(2,2)", lambda x: list(make_tuple(x)),
             lambda x: str(x).replace(",", "-")),

...
class WordsTextualAttributes(AbstractLoader):
    ...
    def __init__(self, **params: Any):
        ...
        # Parse token ids as int (JSON keys are always strings)
        ...
        # Optional part-of-speech tags, loaded the same way
        ...
        # Discover the user/item domain from whatever tokens were actually loaded
    ...
    def load(self) -> Dict[str, Payload]:
        ...
        Returns:
            Dict[str, Payload]: The `tokens`, `users_vocab_embeddings` (if configured),
                and `items_vocab_embeddings` (if configured) payloads.
        """
        payloads: Dict[str, Payload] = {}

        # Merge user and item tokens/ids into one shared id_map (see class docstring)
    ...
    def _vocab_payload(self, path: str) -> EmbeddingPayload:
        ...
        Args:
            path (str): Path to the `.npy` vocabulary/token embedding table.

        Returns:
            EmbeddingPayload: The vocabulary/token embedding payload.
        """
        # MEMORY: copy the whole table into a fresh in-memory array
        ...
        # LAZY: expose a row-at-a-time accessor over the same memory-mapped table
        ...
        # MMAP (default): expose the memory-mapped table directly for bulk access

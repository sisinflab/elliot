...
class ModChainedKG(AbstractLoader):
    def __init__(self, users: t.Set, items: t.Set, ns: SimpleNamespace):
        ...
        self.item_mapping_file = getattr(ns, "item_mapping", None)
        ...
        self.users = users
        self.items = items
        self.item_names = self.load_item_mapping(self.item_mapping_file, "\t")
    ...
    def create_namespace(self):
        ...
        ns.feature_names = self.feature_names
        ns.item_names = self.item_names
    ...
    def load_item_mapping(self, infile, separator='\t'):
        item_names = {}
        with open(infile, encoding='latin-1') as file:
            for line in file:
                line = line.rstrip("\n").split(separator)
                item_names[int(line[0])] = line[1]
        return item_names
    ...
    def reduce_attribute_map_property_selection(self, map, items, feature_names, properties, additive, threshold = 10):
        ...
        features_popularity = {k: v for k, v in feature_occurrences_dict.items() if (1 - v / nitems) <= threshold}

        print(f"Features above threshold:\t{len(features_popularity)}")
        print(f"threshold -> minimum number of occurrences:\t{(1-threshold)*nitems}")

...
class KaHItemKNN(RecMixin, BaseRecommenderModel):
    ...
    def __init__(self, data, config, params, *args, **kwargs):
        ...
        self._model = KaHItemKNNModel(self._user_factors_b.toarray(),
    ...
    def get_single_recommendation(self, mask, k, *args):
        return {u: self._itemknn.get_user_recs(u, mask, k) for u in self._data.train_dict.keys()}

    def get_recommendations(self, k: int = 10):
        self._itemknn = Similarity(data=self._data, attribute_matrix=self._model.get_item_features(),
                                 num_neighbors=self._num_neighbors, similarity=self._similarity,
                                 implicit=self._implicit)
        self._itemknn.initialize()

...
def TF_IDF(dataMatrix):
    ...
    # apply TF-IDF adjustment WRONG
    # dataMatrix.data = np.sqrt(dataMatrix.data) * idf[dataMatrix.col]

    norm = dataMatrix.copy()
    norm.data = idf[dataMatrix.col] ** 2
    norm_sum = np.sqrt(np.array(norm.sum(axis=1)).squeeze())
    norm_sum = norm_sum[dataMatrix.row]
    dataMatrix.data = idf[dataMatrix.col] / norm_sum
...
def UI(dataMatrix):
    ...
    norm_sum = np.sqrt(np.array(norm.sum(axis=1)).squeeze())
    norm_sum = norm_sum[dataMatrix.row]
    dataMatrix.data = dataMatrix.data / norm_sum

...
class LightGCNUUModel(torch.nn.Module, ABC):
    def __init__(self,
                 num_users,
                 num_items,
                 learning_rate,
    ...
    def propagate_embeddings(self, evaluate=False):
        ego_embeddings = self.Gu.to(self.device)
        ...
        gu = sum([all_embeddings[k] * self.alpha[k] for k in range(len(all_embeddings))])
        return gu
    ...
    def predict(self, gu, **kwargs):
        return torch.matmul(gu.to(self.device), torch.transpose(self.Gi.to(self.device), 0, 1))

    def train_step(self, batch):
        gu = self.propagate_embeddings()
        user, pos, neg = batch
        xu_pos = self.forward(inputs=(gu[user[:, 0]], self.Gi[pos[:, 0]]))
        xu_neg = self.forward(inputs=(gu[user[:, 0]], self.Gi[neg[:, 0]]))

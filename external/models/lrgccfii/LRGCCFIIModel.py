...
class LRGCCFIIModel(torch.nn.Module, ABC):
    def __init__(self,
                 num_users,
                 num_items,
                 learning_rate,
    ...
    def propagate_embeddings(self):
        ego_embeddings = self.Gi.weight.to(self.device)
        ...
        gi = torch.cat(all_embeddings, dim=1)
        return gi
    ...
    def predict(self, offset, offset_stop, gi, **kwargs):
        return torch.matmul(self.Gu.weight[offset: offset_stop].to(self.device), torch.transpose(gi.to(self.device), 0, 1))

    def train_step(self, batch):
        gi = self.propagate_embeddings()
        user, pos, neg = batch
        xu_pos, gamma_u, gamma_i_pos = self.forward(inputs=(self.Gu.weight[user], gi[pos]))
        xu_neg, _, gamma_i_neg = self.forward(inputs=(self.Gu.weight[user], gi[neg]))

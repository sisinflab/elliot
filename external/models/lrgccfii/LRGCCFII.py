...
from .LRGCCFIIModel import LRGCCFIIModel

from torch_sparse import SparseTensor


class LRGCCFII(RecMixin, BaseRecommenderModel):
    @init_charger
    ...
    def get_recommendations(self, k: int = 100):
        ...
        self._model.eval()
        with torch.no_grad():
            gi = self._model.propagate_embeddings()
            for index, offset in enumerate(range(0, self._num_users, self._batch_size)):
                offset_stop = min(offset + self._batch_size, self._num_users)
                predictions = self._model.predict(offset, offset_stop, gi)

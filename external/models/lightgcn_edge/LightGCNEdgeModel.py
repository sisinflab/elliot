...
class LightGCNEdgeModel(torch.nn.Module, ABC):
    def __init__(self,
                 num_users,
                 num_items,
                 num_interactions,
        ...
        self.Ge = torch.nn.Parameter(
            torch.nn.init.xavier_normal_(torch.empty((self.num_interactions, self.embed_k))))
    ...
    def propagate_embeddings(self, evaluate=False):
        ...
        edge_embeddings = torch.cat((self.Ge.to(self.device), self.Ge.to(self.device)), 0)

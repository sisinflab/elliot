...
class KaHUserKNN(RecMixin, BaseRecommenderModel):
    ...
    def __init__(self, data, config, params, *args, **kwargs):
        ...
        self._model = KaHUserKNNModel(self._user_factors_b.toarray(),
    ...
    def get_single_recommendation(self, mask, k, *args):
        return {u: self._userknn.get_user_recs(u, mask, k) for u in self._data.train_dict.keys()}

    def get_recommendations(self, k: int = 10):
        # attribute_matrix = self._model.get_user_features()
        a = self._data.sp_i_train @ self._model.get_item_features().numpy()
        b = np.array(self._data.sp_i_train.sum(axis=1)).squeeze()
        attribute_matrix = (a.T / b).T
        self._userknn = Similarity(data=self._data, attribute_matrix=attribute_matrix,
                                 num_neighbors=self._num_neighbors, similarity=self._similarity,
                                 implicit=self._implicit)
        self._userknn.initialize()

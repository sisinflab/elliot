...
from .LightGCNIIModel import LightGCNIIModel

from torch_sparse import SparseTensor


class LightGCNII(RecMixin, BaseRecommenderModel):
    @init_charger
    def __init__(self, data, config, params, *args, **kwargs):
        ...
        self.adj = SparseTensor(row=torch.tensor(np.array(rows_ii), dtype=torch.int64),
                                ...
                                sparse_sizes=(self._num_items,
                                              self._num_items))

        self._model = LightGCNIIModel(
    ...
    def get_recommendations(self, k: int = 100):
        ...
        gi = self._model.propagate_embeddings(evaluate=True)
        for index, offset in enumerate(range(0, self._num_users, self._batch_size)):
            offset_stop = min(offset + self._batch_size, self._num_users)
            predictions = self._model.predict(self._model.Gu[offset: offset_stop], gi)

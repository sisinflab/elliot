...
class LightGCNIIModel(torch.nn.Module, ABC):
    def __init__(self,
                 num_users,
                 num_items,
                 learning_rate,
    ...
    def propagate_embeddings(self, evaluate=False):
        ego_embeddings = self.Gi.to(self.device)
        ...
        gi = sum([all_embeddings[k] * self.alpha[k] for k in range(len(all_embeddings))])
        return gi
    ...
    def train_step(self, batch):
        ...
        user, pos, neg = batch
        xu_pos = self.forward(inputs=(self.Gu[user[:, 0]], gi[pos[:, 0]]))
        xu_neg = self.forward(inputs=(self.Gu[user[:, 0]], gi[neg[:, 0]]))

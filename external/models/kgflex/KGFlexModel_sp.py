...
class KGFlexModel():
    def __init__(self,
        ...
        self.P_sp = dict()
        self.K_sp = dict()
        for u in tqdm(range(self._n_users), desc='Sparse'):
            ...
            self.P_sp[u] = csr_matrix(self.P[u])
            ...
            self.K_sp[u] = csr_matrix(self.K[u])
    ...
    def train_step(self, batch):
        ...
        for us_, mask_pos, mask_neg, d_loss_ in zip(user, m_p, m_n, d_loss):
            ...
            # d = self.K[us_][f_p] * d_loss_
            # s = self.K_sp[us_].T[f_p] * d_loss_
            # d1 = self._learning_rate * self.Gf[f_p]
            # s1 = self._learning_rate * self.Gf[f_p]
            # d2 = d1 * d[:, np.newaxis]
